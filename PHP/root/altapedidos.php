...
<BODY BGCOLOR="#F7F8E0">
	...
	<hr />
	<form name = "clientes" action="altapedidos.php" method="post">
	<p>Escriba la ruta para consulta: <input type="text" name="Ruta" value="" class="text" size="1"></p>
	<p>Seleccionar el estado de los clientes: <input type="radio" name="estatus" value="Activo">Activo<input type="radio" name="estatus" value="Suspendido">Suspendido</p>
	<p>Mostrar Clientes Asignados:<input type="submit" name="mostrar" value="Consulta" class="pedidos" /></p>
	<?php
	session_start();
	...
	if(isset($_REQUEST['mostrar'])) //Si se pulsa el botón de mostrar
	{
 	...
 	// 	while($usuariosasesor = mysql_fetch_array($sqlasesor))
		...
		//Guardamos la entrada de texto ruta en una variable
		$ruta = $_POST['Ruta'];

		//Guardamos el valor de los radiobutton para el estado del cliente en una variable
		...
		//Nuevo query para obtener los datos del cliente por ruta
		$sqlcliente = mysql_query("SELECT * FROM clientes where ClaveAsesor = '$clvasesor' AND Ruta = '$ruta' AND Estatus = '$status';");

		//Creamos una tabla en html con los campos de la tabla mysql
		echo "<table border='2'><tr><td>Rap</td><td>Nombre</td><td>Clave Asesor</td><td>Lista de Precios</td><td>Ruta</td><td>Estatus</td></tr>";
	
		//Recibimos la consulta y la pasamos a un array
		while($clienteasesor = mysql_fetch_array($sqlcliente))
		{
			//Llenamos la tabla
			echo"<tr>";
			echo"<td>".$clienteasesor['Rap']."</td>";
			echo"<td>".$clienteasesor['Nombre']."</td>";
			echo"<td>".$clienteasesor['ClaveAsesor']."</td>";
			echo"<td>".$clienteasesor['Listaprecios']."</td>";
			echo"<td>".$clienteasesor['Ruta']."</td>";
			echo"<td>".$clienteasesor['Estatus']."</td>";
			echo"</tr>";
		}
		echo "</table>";
	}
	...
	<form name = "pedidos" action="altapedidos.php" method="post">
	...
	echo"<br>";
	echo"<br>";
	echo"<h3>Clave del Usuario: ".$clvasesor."</h3>";
	...
	if(isset($_POST['borrarpedido']))
	{
		...
		header("location:altapedidos.php");
	}
	?>
	<hr />
	</form>
