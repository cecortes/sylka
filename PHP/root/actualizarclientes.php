...
<BODY BGCOLOR="#F7F8E0">
	...
	<div class="caja">
		<form method="post" action="" enctype="multipart/form-data">
			<h2>Actualizar la lista de Clientes</h2>
			<input type="file" name="archivo" /><br /><br />
			<input type="submit" value="Guardar en Servidor" name="guardar"/>
			<input type="submit" value="Borrar clientes del Servidor" name="borrar"/>
		</form>
	</div>
</body>
</html>
	...
	if(isset($_POST['guardar']))
	{
		...
		if(in_array($ext, $formatos))
		{
			...
			if(move_uploaded_file($nombreTmpArchivo, "/var/www/archivos/$nombreArchivo"))
			{
				...
				echo "<form method='post' action='actualizarclientes2.php'>";
...
if(isset($_POST['borrar']))
{
	echo "<hr />";
	//Query para mysql
	$borrar=mysql_query('DELETE FROM clientes;');
	if(!$borrar)
	{	
	...
	mysql_close();
	echo "<h2>clientes borrados exitosamente</h2>";
	echo "<hr />";
	echo '<a href="asesor.php"><div style="text-align:left"><div style="font-size:45px;">Regresar al panel de asesores</div></div></a>';
}
?>
